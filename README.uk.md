# Ігри — віджет Android TV для Sunshine / Moonlight

Легкий застосунок для **Android TV**, що виводить бібліотеку ігор з твого ПК — зі стрімінгом
через **[Sunshine](https://github.com/LizardByte/Sunshine)** — прямо на головний екран як
**канал-віджет** Android TV. Вибір гри запускає її через
...
- 🎮 Показує бібліотеку ігор Sunshine картками на **головному екрані** Android TV (preview-канал).
...
| Віджет на екрані | Бібліотека | Вибір ігор |
|---|---|---|
...
```
Цей застосунок ──(власна пара: сертифікат + PIN)──► Sunshine на ПК
...
- **Цей застосунок** парується з Sunshine **окремо** (свій PIN) — лише щоб читати список ігор і обкладинки.
...
- ПК із **Sunshine** та увімкненим **PIN-паруванням**.
- Обидва пристрої в одній мережі.

## Встановлення

1. Завантаж останній **`WidgetAndroidTV-x.y.apk`** зі сторінки [Releases](../../releases).
2. Встанови на Android TV:
   - **ADB:** `adb connect <IP-ТВ>:5555`, далі `adb install WidgetAndroidTV-x.y.apk`, або
   - **USB / файловий менеджер:** скопіюй APK на ТВ і відкрий (увімкни «встановлення з невідомих джерел»).

## Налаштування

1. У Sunshine на ПК відкрий веб-інтерфейс (зазвичай `https://localhost:47990`), тримай напоготові вкладку **PIN**.
2. На ТВ відкрий **Ігри** → рядок **Налаштування** → **Додати / спарувати ПК**.
3. Введи IP ПК → **Підключитися** → з'явиться 4-значний PIN.
4. Введи цей PIN у Sunshine. Список ігор синхронізується автоматично.
...
APK з'явиться в `app/build/outputs/apk/debug/WidgetAndroidTV-debug.apk`.
...
Логіку **парування та HTTP-протоколу** GameStream/Sunshine портовано з
